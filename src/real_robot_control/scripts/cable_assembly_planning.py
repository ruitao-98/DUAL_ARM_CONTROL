...
if __name__ == '__main__':
    ...
    print("1--> 水管 || 2--> 3分3通 || 3--> m12螺母")
    choice = wait_for_choice()
    print('choice = ', choice)

    if choice == 1:
        rospy.set_param("goal_width", -58684) #水管
    ...
    pos = np.array([-314.736, -66.147, 11.328])
    rpy = [-180 * np.pi / 180, 7.237 * np.pi / 180, -108.649 * np.pi / 180]
    ...
    pos = np.array([-103.681, -748.133, 321.853])
    rpy = [-91.125 * np.pi / 180, 4.671 * np.pi / 180, -154.568 * np.pi / 180]
    rotm = from_eular_to_matrix(rpy)
    # 规划交接动作
    ...
    # 抓取第二个水管
    pos = np.array([-310.815, -145.607, 11.3])
    rpy = [172.762 * np.pi / 180, 3.861 * np.pi / 180, -108.92 * np.pi / 180]
    rotm = from_eular_to_matrix(rpy)
    planning_grasping(pos, rotm)
    ...
    pos = np.array([-203.03, -530.2, 171.505])
    rpy = [120.631 * np.pi / 180, -8.959 * np.pi / 180, 18.764 * np.pi / 180]
    rotm = from_eular_to_matrix(rpy)
    planning_assembly(pos, rotm)
